package work.aijiu.onepiece.utils
...
class NavUtil private constructor() {
    ...
    fun NavGraphBuilder.navigationX(
        ...
        navigation(
            RouteConfig.MAIN_PAGE,
            newRoute.toString(),
            emptyList(),
            deepLinks,
            builder
        ).apply {
            enterTransition?.let { enterTransitions[baseRoute] = enterTransition }
            exitTransition?.let { exitTransitions[baseRoute] = exitTransition }
            popEnterTransition?.let { popEnterTransitions[baseRoute] = popEnterTransition }
            popExitTransition?.let { popExitTransitions[baseRoute] = popExitTransition }
        }
    }
...
fun NavGraphBuilder.composableX(
    ...
    addDestination(
        AnimatedComposeNavigator.Destination(provider[AnimatedComposeNavigator::class], content)
            .apply {
                this.route = newRoute.toString()
                newParam.forEachIndexed { _, item ->
